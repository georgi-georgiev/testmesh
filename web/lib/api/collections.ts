...
export async function getCollectionChildren(id: string): Promise<{ children: Collection[] }> {
  const response = await apiClient.get(`/collections/${id}/children`);
  return response.data;
}

// Get collection flows
export async function getCollectionFlows(id: string): Promise<{ flows: Flow[] }> {
  const response = await apiClient.get(`/collections/${id}/flows`);
  return response.data;
}
...
export async function addFlowToCollection(
  ...
  await apiClient.post(`/collections/${collectionId}/flows`, data);
}
...
export async function removeFlowFromCollection(
  ...
  await apiClient.delete(`/collections/${collectionId}/flows/${flowId}`);
}
...
export async function getCollectionAncestors(id: string): Promise<{ ancestors: Collection[] }> {
  const response = await apiClient.get(`/collections/${id}/ancestors`);
  return response.data;
}
...
export async function moveCollection(
  ...
  const response = await apiClient.post(`/collections/${id}/move`, data);
  return response.data;
}
...
export async function duplicateCollection(
  ...
  const response = await apiClient.post(`/collections/${id}/duplicate`, { name: newName });
  return response.data;
}
...
export async function reorderCollectionItems(
  ...
  await apiClient.post(`/collections/${collectionId}/reorder`, data);
}
